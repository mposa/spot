...
%            end
        ...
        function [sol] = minimize(prog,pobj,solver,options)
            ...
            if 0
                ...
                [P,A,b,c,K,d] = dl.toSedumi(-dobj, options);
...
%                   keyboard
    ...
    methods (Access = public)
        function [P,A,b,c,K,d] = toSedumi(pr,pobj, options)
            ...
            if isfield(options,'reg_cost') && options.reg_cost
              [~,reg_id] = isfree(options.reg_var);
              [~,free_id] = isfree(pr.freeVar);
              j = find(free_id == reg_id);
              
              %socp constraints first
              I = nf+nl + [1 1+cumsum(K.q(1:end-1))];
              A(:,j) = A(:,j) - sum(A(:,I),2);
              
              for i=1:length(K.s),
                I = nf+nl+nq+nr + sum(K.s(1:i-1).^2) + (1:K.s(i)+1:K.s(i)^2);
                A(:,j) = A(:,j) - sum(A(:,I),2);
              end
            elseif isfield(options,'regularize') && options.regularize
              reg_eps = options.regularize_eps;
              
              %socp constraints first
              I = nf+nl + [1 1+cumsum(K.q(1:end-1))];
              b = b + sum(A(:,I)*reg_eps,2);
